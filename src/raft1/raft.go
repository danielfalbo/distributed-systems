...
type Raft struct {
  ...
  // set by Kill()
	...
	votedFor	int // index among peers array

	// Election target: timestamp when we should start an election.
	electionTarget time.Time
...
// It is up to the caller to ensure to hold the rf.mu lock.
...
func (rf *Raft) resetElectionTarget() {
	ms := 300 + (rand.Int63() % 300)
	rf.electionTarget = time.Now().Add(time.Duration(ms) * time.Millisecond)
}
...
func (rf *Raft) RequestVote(args *RequestVoteArgs, reply *RequestVoteReply) {
	...
	if rf.votedFor != -1 && rf.votedFor != args.CandidateId {
		...
		rf.resetElectionTarget() // We just voted, so we reset the election timer.
	}
}
...
func (rf *Raft) ticker() {
	for rf.killed() == false {
		...
		if rf.role == 2 { // leader
			...
			isElectionTime := time.Now().After(rf.electionTarget)
